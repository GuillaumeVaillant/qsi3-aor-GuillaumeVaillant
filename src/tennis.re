...
let string_of_score = score => 
    switch (score) {
    | Points(points) => "PlayerOne : " ++ string_of_point(points.playerOne) ++
      "PlayerTwo : " ++ string_of_point(points.playerTwo)
    | Forty(forty) => string_of_player(forty.player) ++ "40 "
      ++ "OtherPLayerPoint : " ++ string_of_point(forty.otherPlayerPoint)
    | Deuce => "40 - 40"
    | Advantage(player) => "Advantage " ++ string_of_player(player)
    | Game(player) => "Game " ++string_of_player(player)
    };